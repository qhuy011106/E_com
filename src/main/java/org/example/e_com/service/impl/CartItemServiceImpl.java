...
public class CartItemServiceImpl implements CartItemService {
    ...
    public boolean addCartItem(CartItem cartItem) {
        ...
        return cartItemDao.insert(cartItem) > 0;  // int → boolean
    }
    ...
    public boolean updateCartItem(CartItem cartItem) {
        ...
        return cartItemDao.update(cartItem) > 0;  // int → boolean
    }
    ...
    public boolean deleteCartItem(int id) {
        ...
        return cartItemDao.delete(id) > 0;  // int → boolean
    }

    @Override
    public boolean clearCart(int cartId) {
        if (cartId <= 0) return false;
        return cartItemDao.deleteByCartId(cartId) > 0;  // int → boolean
    }

    @Override
    public int deleteByCartId(int cartId, Connection conn) {
        if (cartId <= 0 || conn == null) return 0;
